...
namespace Linq
{
    ...
    public static class PartitioningData
    {
        ...
        public static IEnumerable<(string customerId, int orderId, DateTime orderDate)> CustomersTake()
        {
            ...
            {
                Console.WriteLine($"CustomerId: {customer.CustomerId}, City: {customer.Region}, ");
            }
        ...
        public static IEnumerable<(string customerId, int orderId, DateTime orderDate)> CustomersSkip()
        {
            ...
            return customers
            .Where(c => c.Region == "WA")
            .SelectMany(c => c.Orders.Skip(2).Select(o => (c.CustomerId, o.OrderId, o.OrderDate)));
        }
        ...
        public static IEnumerable<int> IndexedTakeWhile()
        {
            ...
            return numbers.Select((value, index) => new { value, index }).TakeWhile(x => x.value >= x.index).Select(x => x.value); 
        }
        ...
        public static IEnumerable<int> IndexedSkipWhile()
        {
            ...
            return numbers.Select((value, index) => new { value, index }).SkipWhile(x => x.value >= x.index).Select(x => x.value);
        }
    }
}